...
## Event Dashboard: 
### Two tailored management views:

My Events: 
For coordinators to monitor seat occupancy meters and access immediate updates.

My Bookings:
For attendees to track reservations they have committed to.

## Robust Input Validation: Dual-layer validation 
(HTML5 frontend constraints and Django form clean methods) preventing zero or negative values in seat configurations.
...
### Email Confirmations:
 Automated, real-time dispatch of ticketing profiles (venue, time routing, seat allocations) upon booking or cancellation actions.
...
Create a file named .env in the root project directory (where manage.py lives).
 ...
 CRITICAL FORMATTING: Do NOT use quotation marks around strings or insert empty spaces around the = signs. Google App Passwords must be a solid, contiguous 16-character string without spaces.
...
EMAIL_HOST_PASSWORD= one time password generated from google
...
Open your browser and navigate to [http://127.0.0.1:8000/](http://127.0.0.1:8000/) to explore EventHive!
...
accounts/: Manages operational profile registration, sign-in wrappers, and auth sequences.
